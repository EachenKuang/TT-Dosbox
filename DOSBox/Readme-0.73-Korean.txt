...
물음: 제 빌드 게임(Duke3D/Blood/Shadow Warrior)에 문제가 있습니다.
...
풀이: DOSBox에서 CD-ROM을 띄우려면 CD-ROM을 띄울 때 몇 가지 또 다른 선택 항목을
   ...
   ① MSCDEX를 비롯한 CD-ROM 지원을 사용하려면:
       ...
       - mount d /media/cdrom -t cdrom (리눅스)
      이따금씩은 특수한 CD-ROM 인터페이스를 사용하여야 하는 경우도 있습니다.
      이를테면 CD 오디오가 동작하지 않으면:
       SDL 지원 방식을 사용하거나
       - mount d f:＼ -t cdrom -usecd 0 -noioctl
       CD 오디오의 디지털 오디오 확장을 사용한 ioctl 접근을 사용하거나 (Windows 전용, 비스타에 적합)
...
풀이: ＼ 는 \와 같습니다. 한국어 자판을 쓸 때에는 이와 같은 문제가 없습니다.
  ...
  : (콜론)의 경우 Shift 키를, 또 Enter와 I 사이의 키들을 눌러 보십시오.
...
물음 : 제 빌드 게임(Duke3D/Blood/Shadow Warrior)에 문제가 있습니다.
풀이 : 먼저 게임 포트를 찾아 보십시오. 그것이 게임을 하는 데 더 좋은 체험을 제공합니다.
        DOSBox.Conf를 열고 machine=svga_s3를 찾아서 vesa_nolfb로 바꾸어 보십시오.
...
dosbox -version
  ...
  -editconf 프로그램
        ...
        하나 이상의 명령을 지정할 수 있습니다. 이 경우 첫 번째 프로그램이 시작을 실패하면 두 번째 프로그램이 실행됩니다.

  -opencaptures 프로그램
        첫 변수로 지정된 프로그램을 캡처 폴더로 호출합니다.
...
IMGMOUNT 드라이브 [이미지 파일1, 이미지 파일2, ... 이미지파일N] -t iso -fs iso

  이미지 파일
      DOSBox에서 마운트하려는 이미지 파일의 위치입니다.
      이 위치는 DOSBox 안에서 마운트된 드라이브나 실제 디스크로 설정할 수 있습니다.
      CD-ROM 이미지 (ISO 또는 CUE/BIN) 또한 마운트할 수 있습니다. CD 바꾸기 기능을 사용하고 싶으시면
      바로 아래에 있는 옵션을 보십시오.
...
이것은 기본적인 키보드 조합입니다. 매핑 프로그램(Ctrl-F1)에서 바꿀 수 있습니다. (단락 6 참조)
